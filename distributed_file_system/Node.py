...
if __name__ == '__main__':
	...
	parser = configparser.ConfigParser()
	parser.read('config.cfg')
	if node >= int(parser['Quorum']['node_count']):
		print("Please provide valid node number within node count")
		sys.exit()
	#get server port number from config file
	ports = getPorts('Node').rstrip("\n").split(", ")
	addresses = int(len(parser['Node']['address'].rstrip("\n").split(", ")))
	if len(ports) != int(parser['Quorum']['node_count']) or addresses != int(parser['Quorum']['node_count']):
	...
	processor = QuorumService.Processor(handler)
